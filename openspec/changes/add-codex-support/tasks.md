...
- [x] 1.1 在 `aide/src/cli/init.rs` 中添加 `sync_plugins_to_codex` 函数
  - 处理全局目录：`~/.codex/prompts/`
  - 处理项目目录：`.agents/skills/`
  - 复用 `copy_dir_all` 函数
  - 添加错误处理和日志输出

- [x] 1.2 在 `handle_init` 函数中集成 Codex 同步
  - 在 `sync_plugins_to_project` 之后调用
  - 确保不影响现有的 Claude Code 同步流程

- [x] 1.3 在 `handle_init_global` 函数中集成 Codex 同步
...
- [x] 3.1 更新用户文档说明 Codex 支持
- [x] 3.2 在初始化输出中添加 Codex 同步的提示信息
